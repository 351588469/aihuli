...
public class GMUserController extends BaseController {
	...
	public ModelAndView exportExcel(HttpServletRequest request,Page page) throws Exception{
		...
		if(!pd.containsKey("all"))
			varOList=gmuserService.list(page);	//列出Elder列表
		else 
			varOList=gmuserService.listAll(pd);
		
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			String gmid=varOList.get(i).getString("GMU_GM_ID");
			...
			String duties=varOList.get(i).get("GMU_DUTIES").toString();
			String y_duties="";
			if(duties.equals("1"))y_duties="护士长";
			if(duties.equals("2"))y_duties="护士";
			vpd.put("var6",y_duties);	//6
			...
			String estatus= varOList.get(i).get("GMU_ESTATUS").toString();
			String y_estatus="";
			if(estatus.equals("1")) y_estatus="入职";
			else if(estatus.equals("2")) y_estatus="离职";
			else if(estatus.equals("3")) y_estatus="请假";
			vpd.put("var13", y_estatus);	//13
